    ...
    public void build() {
        ...
        for (int tubeIndex = -tubeDivisions; tubeIndex < tubeDivisions; tubeIndex++) {
            ...
            for (int radiusIndex = -radiusDivisions; radiusIndex < radiusDivisions; radiusIndex++) {
                ...
                points[pointIndex + 0] = x = (float) (aProperty.getValue() * 1 * (Math.sinh(tubeIndex)) * (Math.cos(v)));
                points[pointIndex + 1] = y = (float) (bProperty.getValue() * 1 * (Math.sinh(tubeIndex) * (Math.sin(v))));
                points[pointIndex + 2] = z = (float) (cProperty.getValue() * (Math.cosh(tubeIndex)));
